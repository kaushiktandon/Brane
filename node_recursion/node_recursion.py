...
def load_nodesJSON(nodes_file,title_split, invalid_ids):
	...
	with open(nodes_file) as f:
		for line in f:
			...
			if(a_id < 91567675 and title_split == 0):
				continue
			a_id = str(a_id)
			...
			if(title_split != 0):
				...
				if(invalid_ids != None and a_id not in invalid_ids and '(Project)' in name or '(Mitacs)' in name):
					if(definition != "" and definition != " " and definition != "Enter a definition" and definition != "Add a definition"):
						nodes.append(Node(name,a_id,definition))	
				elif(invalid_ids == None and '(Project)' in name or '(Mitacs)' in name):
					if(definition != "" and definition != " " and definition != "Enter a definition" and definition != "Add a definition"):
						nodes.append(Node(name,a_id,definition))
	print ("Loaded nodes")
	return nodes
...
if __name__ == '__main__':
	if(len(sys.argv) != 4):
		...
		no_include = loadList('companies_converted.csv')
		invalid_ids = no_include + ['84863573', '84863703','85165547','84865090','84873582','84865696','84863588','84863730','84864590','85166489','84878834','84863710','84864681','85170352','85165586','84867280','85165549','85165556','85165553','85165563','84873191','85165548','85165570']
		nodes = load_nodesJSON(nodes_file,0, no_include)
		print(len(nodes))
		links = load_linksJSON(links_file)
		
		potential_nodes_to_link = load_nodesJSON(nodes_file,2, invalid_ids)
		...
		with open(created_CSV_file, 'w+') as csvfile:
			...
			for temp in nodes:
				...
				for temp2 in potential_nodes_to_link:
					if(temp2.ID != node_ID):
						a = temp2.name
						if(len(a.split(" ")) < 2):# and a not in valid_nodes): #Current doing 2 gram
							continue
						a = a.lower()
