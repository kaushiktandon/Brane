...
output_file = "nasa_mini_scrape.csv"
...
def createNodeWithLink(name,id,parentCluster,output_file,url=None):
	with open(output_file, 'a+') as csvfile:
		...
		if(url == None):
			writer.writerow(['CN',str(id),name])
		else:
			writer.writerow(['CN',str(id),name,'reference',url])
		writer.writerow(['CL',parentCluster,str(id),'is a kind of', 'contains'])
	return id + 1
...
def createTechnicalReportNodes(output_file,id):
	startTime =  time.time()
	print startTime
	all_authors = dict()
	resumptionTokenStart = 1
	while(resumptionTokenStart <= 101):
		...
		print(str(responseTime) + ": " + str(responseTime - requestTime) + " seconds to get response.")
...
def main():
	...
	createTechnicalReportNodes(output_file,idCount)
